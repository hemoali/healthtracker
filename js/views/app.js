...
app.AppView = Backbone.View.extend({
	el: '#container',
	daysTemplate: _.template($('#days-item-template').html()),
	events: {
		// Search button excutes searchData function
		'click #search-button': 'searchData'
	},
	initialize: function (options) {
		// Cache views
		this.search_text = this.$('#search-text');
		this.total_calories = this.$('#total-calories');
		this.daysNav = $("#nav");
	...
	renderDays: function () {
		...
		for (var i = 1; i <= this.lastDay; i++) {
			this.daysNav.append(this.daysTemplate({
				i: i,
				active: (i == this.lastDay) ? 'active' : ''
			}));
		}
		// Add the last add_day tab
		this.daysNav.append('<a class="day-link" id="add-day" href="#">+Day</a>');
		...
		$("#add-day").click($.proxy(this.addNewDay, this));
	},
	/**
	...
	addItemToSavedItems: function (item) {
		...
		$('#saved-items-list').append(view.render().el);
		this.filterSavedItems();
	},
	/**
	...
	loadDataFromAPI: function (search_text) {
		...
		$('#search-results-loadingview').removeAttr('hidden');
		$.getJSON("https://api.nutritionix.com/v1_1/search/" + search_text + "?fields=item_name,brand_name,item_id,nf_calories&appId=cbc8cc2e&appKey=b0e81dc2410863002692d2f1685220e5", function (result) {
			...
			$('#search-results-loadingview').attr('hidden', 'hidden');
		}).fail(function () {
			// If there's error connecting to the service
			alert("Something went wrong, please check your internet connection");
			// Hide the progress bar of the search list
			$('#search-results-loadingview').removeAttr('hidden', 'hidden');
		});
	},
	/**
